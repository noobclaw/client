...
export class IMCoworkHandler extends EventEmitter {
  ...
  private buildSessionTitle(
    ...
    // Show "Lark" instead of "feishu" when domain is lark
    let displayPlatform: string = platform;
    if (platform === 'feishu') {
      try {
        const feishuConfig = this.imStore.getConfig()?.feishu;
        if (feishuConfig?.domain === 'lark') displayPlatform = 'Lark';
      } catch {}
    }
    return `IM-${displayPlatform}-${Date.now()}`;
  }
