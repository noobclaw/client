...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.NOOBCLAW_ELECTRON_PATH = getElectronNodeRuntimePath();
...
const downloadGithubArchive = async (
  ...
  archiveUrlCandidates.push({
    ...
    headers: {
      ...
      'User-Agent': 'NoobClaw Skill Downloader',
...
async function downloadMdUrl(mdUrl: string, destDir: string): Promise<string> {
  console.info('[skill-manager] Downloading SKILL.md from:', mdUrl);

  // Use Node.js https directly (most reliable in packaged Electron apps)
  const https = require('https');
  const http = require('http');
  const content: string = await new Promise<string>((resolve, reject) => {
    const mod = mdUrl.startsWith('https') ? https : http;
    const req = mod.get(mdUrl, { headers: { 'User-Agent': 'NoobClaw/1.0' }, timeout: 30000 }, (res: any) => {
      // Follow redirects
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        const redirectUrl = res.headers.location;
        console.info('[skill-manager] Following redirect to:', redirectUrl);
        const mod2 = redirectUrl.startsWith('https') ? https : http;
        mod2.get(redirectUrl, { headers: { 'User-Agent': 'NoobClaw/1.0' } }, (res2: any) => {
      ...
      res.on('end', () => {
        console.info('[skill-manager] Download complete, size:', Buffer.concat(chunks).length, 'bytes');
        resolve(Buffer.concat(chunks).toString('utf-8'));
      });
      res.on('error', reject);
    });
    req.on('error', (err: Error) => {
      console.error('[skill-manager] Download error:', err.message);
      reject(err);
    });
    req.on('timeout', () => {
      console.error('[skill-manager] Download timeout');
      req.destroy();
  ...
  // Derive a meaningful folder name from the URL or the SKILL.md frontmatter name
  let folderName = 'skill-md';
  try {
    const { frontmatter } = parseFrontmatter(content);
    if (typeof frontmatter.name === 'string' && frontmatter.name.trim()) {
      folderName = normalizeFolderName(frontmatter.name.trim());
    }
  } catch { /* ignore parse errors */ }
  if (folderName === 'skill-md' || !folderName) {
    // Fallback: derive from URL path (e.g. ".../skills/binance/spot.md" → "spot")
    const urlPath = mdUrl.replace(/[#?].*$/, '').replace(/\/+$/, '');
    const baseName = urlPath.split('/').pop()?.replace(/\.md$/i, '') || '';
    const normalized = normalizeFolderName(baseName);
    if (normalized && normalized !== 'skill') {
      folderName = normalized;
    }
  }
  const skillDir = path.join(destDir, folderName);
  fs.mkdirSync(skillDir, { recursive: true });
  fs.writeFileSync(path.join(skillDir, 'SKILL.md'), content, 'utf-8');
  console.info('[skill-manager] Downloaded SKILL.md to', skillDir, '(' + content.length + ' bytes)');
  return skillDir;
}
...
export class SkillManager {
    ...
    try {
          ...
          if (githubSource) {
            try {
              ...
              if (errno === 'ENOENT' && process.platform === 'win32') {
                throw new Error(
                  'Git executable not found. Please install Git for Windows or reinstall NoobClaw with bundled PortableGit.'
            ...
            throw new Error('Git executable not found. Please install Git for Windows or reinstall NoobClaw with bundled PortableGit.');
          } else {
            throw error;
          }
        }
      ...
      for (let idx = 0; idx < skillDirs.length; idx++) {
        ...
        if (fs.existsSync(targetDir)) {
          ...
          console.info('[skill-manager] Skill folder already exists, updating:', folderName);
          fs.rmSync(targetDir, { recursive: true, force: true });
        }
        cpRecursiveSync(skillDir, targetDir);

        // Inject marketplace metadata (e.g. official flag) into SKILL.md frontmatter
        if (meta?.official) {
          try {
            const targetSkillFile = path.join(targetDir, SKILL_FILE_NAME);
            if (fs.existsSync(targetSkillFile)) {
              const raw = fs.readFileSync(targetSkillFile, 'utf8');
              if (!raw.includes('official:')) {
                // Insert `official: true` after the opening `---`
                const updated = raw.replace(/^---\s*\n/, '---\nofficial: true\n');
                fs.writeFileSync(targetSkillFile, updated, 'utf8');
              }
            }
          } catch (e) {
            console.warn('[skill-manager] Failed to inject official flag:', e);
          }
        }
      }
  ...
  private parseSkillDir(
    ...
    try {
      ...
      const name_zh = typeof frontmatter.name_zh === 'string' ? frontmatter.name_zh.trim() : undefined;
      const description_zh = typeof frontmatter.description_zh === 'string' ? frontmatter.description_zh.trim() : undefined;
      ...
      const version = typeof frontmatter.version === 'string' ? frontmatter.version
        : typeof frontmatter.version === 'number' ? String(frontmatter.version)
        : undefined;
      // Extract author from top-level or metadata.author
      const metadataObj = frontmatter.metadata && typeof frontmatter.metadata === 'object' && !Array.isArray(frontmatter.metadata)
        ? frontmatter.metadata as Record<string, unknown>
        : null;
      const author = typeof frontmatter.author === 'string' ? frontmatter.author.trim()
        : (metadataObj && typeof metadataObj.author === 'string') ? metadataObj.author.trim()
        : undefined;
      ...
      return { id, name, description, enabled, isOfficial, isBuiltIn, updatedAt, prompt, skillPath: skillFile, version, name_zh, description_zh, author };
    } catch (error) {
