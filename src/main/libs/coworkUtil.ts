...
function ensureElectronNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
    ...
    const nodeShContent = [
      '#!/usr/bin/env bash',
      'if [ -z "${NOOBCLAW_ELECTRON_PATH:-}" ]; then',
      '  echo "NOOBCLAW_ELECTRON_PATH is not set" >&2',
      '  exit 127',
      'fi',
      'exec env ELECTRON_RUN_AS_NODE=1 "${NOOBCLAW_ELECTRON_PATH}" "$@"',
      '',
    ].join('\n');
    ...
    if (process.platform === 'win32') {
      ...
      const nodeCmdContent = [
        '@echo off',
        'if "%NOOBCLAW_ELECTRON_PATH%"=="" (',
        '  echo NOOBCLAW_ELECTRON_PATH is not set 1>&2',
        '  exit /b 127',
        ')',
        'set ELECTRON_RUN_AS_NODE=1',
        '"%NOOBCLAW_ELECTRON_PATH%" %*',
        '',
      ].join('\r\n');
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        // npx.cmd for Windows — uses %NOOBCLAW_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          const npxCmd = join(shimDir, 'npx.cmd');
          const npxCmdContent = [
            '@echo off',
            '"%~dp0node.cmd" "%NOOBCLAW_NPM_BIN_DIR%\\npx-cli.js" %*',
            '',
          ].join('\r\n');
          writeFileSync(npxCmd, npxCmdContent, 'utf8');
          coworkLog('INFO', 'resolveNodeShim', `Created npx.cmd shim: ${npxCmd} (using env var NOOBCLAW_NPM_BIN_DIR)`);
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %NOOBCLAW_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog('INFO', 'resolveNodeShim', `Created npm.cmd shim: ${npmCmd} (using env var NOOBCLAW_NPM_BIN_DIR)`);
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in NoobClaw package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by NoobClaw – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.NOOBCLAW_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.NOOBCLAW_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.NOOBCLAW_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  env.NOOBCLAW_NPM_BIN_DIR = npmBinDir;
  ...
  if (shouldInjectShim) {
    ...
    if (shimDir) {
      ...
      env.NOOBCLAW_NODE_SHIM_ACTIVE = '1';
    ...
    delete env.NOOBCLAW_NODE_SHIM_ACTIVE;
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.NOOBCLAW_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `NOOBCLAW_ELECTRON_PATH=${env.NOOBCLAW_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `NOOBCLAW_NPM_BIN_DIR=${env.NOOBCLAW_NPM_BIN_DIR || '(not set)'}`);
...
export function getSkillsRoot(): string {
  ...
  const envRoots = [process.env.NOOBCLAW_SKILLS_ROOT, process.env.SKILLS_ROOT]
...
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local'): Promise<Record<string, string | undefined>> {
  ...
  env.NOOBCLAW_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
  if (process.platform === 'win32' || env.NOOBCLAW_NODE_SHIM_ACTIVE === '1') {
    env.NOOBCLAW_ELECTRON_PATH = getElectronNodeRuntimePath().replace(/\\/g, '/');
  } else {
    delete env.NOOBCLAW_ELECTRON_PATH;
  }
...
export async function probeCoworkModelReadiness(
  ...
  if (!config) {
    return {
      ok: false,
      error: resolution.error || 'API configuration not found.',
    };
  }

  // Skip probe for NoobClaw AI service — our backend is always available
  // and the probe can fail due to proxy initialization timing
  if (config.model?.startsWith('noobclawai-')) {
    return { ok: true };
  }
  ...
  try {
    ...
    if (config.apiType === 'openai') {
      ...
      body = { model: config.model, max_tokens: 1, temperature: 0, stream: false, messages: [{ role: 'user', content: 'Reply with "ok".' }] };
    } else {
      url = buildAnthropicMessagesUrl(config.baseURL);
      headers['x-api-key'] = config.apiKey;
      headers['anthropic-version'] = '2023-06-01';
      body = { model: config.model, max_tokens: 1, temperature: 0, messages: [{ role: 'user', content: 'Reply with "ok".' }] };
    }
...
export async function generateSessionTitle(userIntent: string | null): Promise<string> {
  ...
  try {
    ...
    if (config.apiType === 'openai') {
      // When apiType is 'openai', baseURL points to the local proxy which only
      // accepts Anthropic format at /v1/messages and translates to OpenAI upstream.
      ...
      if (!response.ok) {
        console.warn('[cowork-title] OpenAI title gen failed:', response.status);
        return fallbackTitle;
      }

      const payload = await response.json();
      llmTitle = extractTextFromAnthropicResponse(payload);
    } else {
      // Anthropic format
      const url = buildAnthropicMessagesUrl(config.baseURL);
      const response = await fetch(url, {
      ...
      if (!response.ok) {
        console.warn('[cowork-title] Anthropic title gen failed:', response.status);
        return fallbackTitle;
      }
