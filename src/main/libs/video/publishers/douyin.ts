...
async function commitHashtagPill(editorSel: string, tag: string): Promise<boolean> {
  const clean = tag.replace(/[#\s,，]+/g, '');
  if (!clean) return false;
  try {
    // 1. 粘 "#tag" 触发 dropdown
    await sendBrowserCommand('editor_paste_text', {
      selector: editorSel,
      text: '#' + clean,
    ...
    try {
      await sendBrowserCommand('main_world_click',
        { selector: dropdownSel }, 5000, bridgeOptsFor(PLATFORM));
      return true;
    } catch {
  ...
  try {
    ...
    if (input.tags && input.tags.length) {
      ...
      for (const tag of input.tags.slice(0, 5)) {
        await commitHashtagPill(editorSel, tag);
        await sleep(250);
      }
    }
