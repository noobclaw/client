...
export class CoworkRunner extends EventEmitter {
  ...
  private composeEffectiveSystemPrompt(
    ...
    const coreRulesPrompt = [
      ...
      '',
      '### Tool Routing (IMPORTANT — follow strictly):',
      '- Open website/URL → browser_navigate (NEVER Bash). Browser extension has user login sessions.',
      '- Open desktop app → desktop_open_app. Then desktop_screenshot to verify.',
      '- Interact with web page → browser_click/browser_type (NEVER Bash curl).',
      '- Interact with desktop app → desktop_screenshot then desktop_click/desktop_type.',
      '- Search the web → web_search (NEVER Bash curl/wget).',
      '- Read/write files → Read/Write/Edit (NEVER Bash cat/echo/sed).',
      '- Run commands → Bash (ONLY for shell commands that have no dedicated tool).',
      '- If a tool fails, try ONE alternative. If that also fails, ask the user — do NOT loop.',
      '',
      '## Output',
