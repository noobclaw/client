  ...
  private async sendNotifications(
    ...
    for (const platform of task.notifyPlatforms) {
      try {
        ...
        // Map 'lark' to 'feishu' gateway (same gateway, different domain)
        const gwPlatform = platform === 'lark' ? 'feishu' : platform;
        await imManager.sendNotificationWithMedia(gwPlatform as any, message);
