  ...
  const ctx: Record<string, any> = {
    ...
    downloadWithYtDlp: async (
      ...
      try {
        ...
        // 4. 抓浏览器 cookies 写 Netscape 格式 tmpfile —— 抖音 detail
        //    API 必须带 HttpOnly cookies (msToken / ttwid / __ac_nonce /
        ...
        try {
          ...
          if (cookies.length > 0) {
            ...
            const lines = ['# Netscape HTTP Cookie File', '# Generated by noobclaw phaseRunner'];
            for (const c of cookies) {
              const domain = c.domain || '';
              const flag = domain.startsWith('.') ? 'TRUE' : 'FALSE';
              const cpath = c.path || '/';
              const secure = c.secure ? 'TRUE' : 'FALSE';
              // CDP returns expires as seconds-since-epoch (float) or -1 for session;
              // Netscape format wants integer, 0 for session cookies.
              const exp = (c.expires && c.expires > 0) ? Math.floor(c.expires) : 0;
              const name = c.name || '';
              const value = c.value || '';
              // tab-separated, no quoting
              lines.push([domain, flag, cpath, secure, exp, name, value].join('\t'));
            }
            cookiesFile = path.join(os.tmpdir(), 'noobclaw_ytdlp_cookies_' + Date.now() + '.txt');
            fs.writeFileSync(cookiesFile, lines.join('\n') + '\n', 'utf8');
            coworkLog('INFO', 'phaseRunner', 'downloadWithYtDlp cookies prepared', {
              cookieCount: cookies.length, cookiesFile,
            });
          } else {
        ...
        // 5. 调用 yt-dlp。stderr 留着抓错误日志,stdout 不重要。
        //    --no-warnings 关掉 yt-dlp 自己的 warning 噪音。
        //    --no-playlist 防止 detail URL 被识别成 playlist 拉 N 个。
        //    --cookies <file> 喂浏览器 cookies。
        const cookiesFlag = cookiesFile ? ` --cookies "${cookiesFile}"` : '';
        try {
          execSync(
            `"${ytdlpCmd}" --no-warnings --no-playlist${cookiesFlag} -f ${format} -o "${outPath}" "${sourceUrl}"`,
