...
const getDefaultWorkingDirectory = (): string => {
  return path.join(os.homedir(), 'noobclaw', 'project');
};
...
export class CoworkStore {
  ...
  getConfig(): CoworkConfig {
    ...
    const dangerouslySkipPermissionsRow = this.getOne<ConfigRow>('SELECT value FROM cowork_config WHERE key = ?', ['dangerouslySkipPermissions']);
    ...
    return {
      ...
      dangerouslySkipPermissions: parseBooleanConfig(dangerouslySkipPermissionsRow?.value, DEFAULT_DANGEROUSLY_SKIP_PERMISSIONS),
    };
  }
  ...
  setConfig(config: CoworkConfigUpdate): void {
    ...
    if (config.dangerouslySkipPermissions !== undefined) {
      this.db.run(`
        INSERT INTO cowork_config (key, value, updated_at)
        ...
        ON CONFLICT(key) DO UPDATE SET
          value = excluded.value,
          updated_at = excluded.updated_at
      `, [config.dangerouslySkipPermissions ? '1' : '0', now]);
    }
  ...
  getAppLanguage(): 'zh' | 'en' {
    const lang = this.getAppLanguageFull();
    return lang === 'en' ? 'en' : 'zh';
  }

  getAppLanguageFull(): string {
    interface KvRow {
      value: string;
    }
