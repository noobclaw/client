/**
 * TikTokConfigWizard — 独立 3-step wizard 模仿 ConfigWizard 的形态:
 *
 ...
 * (TikTok 互动逻辑就是 explore 页随机挑视频按 toggle 做动作,跟 youtube 同款,
 * 不需要"每次运行 N 个动作"的范围)。
 *
 * 跟其他平台 wizard 的字段隔离 — 不读 X 的 KOL pool / Binance 的 track,
 * 也不写这些字段到 task,完全独立避免 UI 串台。在 ConfigWizard.tsx 顶部
 * 通过 `if (scenario.id === 'tiktok_auto_engage') return <TikTokConfigWizard/>`
 * 短路路由到这里。
 *
 ...
 *   - subscribe → follow
 *   - emoji 📺 → 🎵
 *   - 评论描述提到"语言匹配视频与评论区"(TikTok 多语言素材混合,跨语言回复
 *     看着假),其它一致。
 */
      ...
      <div className="w-full max-w-2xl max-h-[90vh] rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 shrink-0">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs px-2.5 py-1 rounded-full border border-pink-500/40 text-pink-500 bg-pink-500/5">
            ...
            <>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-32 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-pink-500/40"
                  disabled={saving}
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
                  {isZh
                    ? '建议 3-8 个 / 天。TikTok 风控比 YouTube 还紧,过多易触发。每个视频会模拟观看停留再操作。'
                    : 'Recommended 3-8/day. TikTok anti-automation is even stricter than YouTube; the bot will simulate dwell time per video before acting.'}
                </p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-xs text-amber-700 dark:text-amber-300 leading-relaxed space-y-1">
                ...
                <ul className="list-disc list-inside space-y-0.5">
                  <li>{isZh ? '关注默认关闭 — TikTok 对自动关注的检测最严,只在确实想关注作者时再开' : 'Follow is off by default — TikTok flags auto-follows most aggressively'}</li>
            ...
            <>
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md text-xs border transition-colors ${
                        runInterval === opt.value
                          ? 'border-pink-500 bg-pink-500/10 text-pink-500 font-medium'
                          : 'border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-pink-500/50'
                      }`}
                    >
                      {opt.label}
                    </button>
              ...
              <label className="flex items-start gap-2.5 cursor-pointer">
                <input
                  ...
                  disabled={saving}
                  className="mt-0.5 h-4 w-4 accent-pink-500 cursor-pointer shrink-0"
                />
                <span className="text-xs leading-relaxed text-gray-600 dark:text-gray-400">
                  {isZh
                    ...
                    : 'I understand TikTok automation carries account risk. The bot will follow the configured cadence with humanized timing. I can stop the task anytime; runs are visible in Run History.'}
                </span>
              </label>
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex items-center gap-2 shrink-0">
          ...
          {step < 3 ? (
            <button
              ...
              disabled={saving}
              className="px-4 py-2 rounded-lg text-sm font-semibold bg-pink-500 text-white hover:bg-pink-600 disabled:opacity-50"
            ...
            <button
              ...
              className="px-5 py-2 rounded-lg text-sm font-semibold bg-pink-500 text-white hover:bg-pink-600 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {saving
                ? (isZh ? '保存中...' : 'Saving...')
