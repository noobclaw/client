        ...
        {loading && tasks.length === 0 ? (
          ...
          <div className="space-y-3">
            {sortedTasks.map(task => {
              ...
              // Twitter: 推特 · 自动互动 / 推特 · 自动发推 / 指定链接仿写
              // XHS:     小红书 · 爆款批量仿写 / 小红书 · 指定链接爆款仿写 / 小红书 · 自动互动
              const typeLabel = (() => {
                if (sid === 'x_auto_engage')                  return { icon: '🐦', zh: '推特 · 自动互动', en: 'Twitter Auto Engage', color: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/30' };
                ...
                if (sid === 'binance_square_auto_engage')     return { icon: '🤝', zh: '币安广场 · 自动互动', en: 'Binance Square Auto Engage', color: 'text-yellow-500 bg-yellow-500/10 border-yellow-500/30' };
                ...
                // into the XHS branch and get tagged 小红书 · 自动互动 (bug observed
                // in 2.4.56). Platform-first guard fixes it.
                const plat = scenario?.platform;
                if ((scenario?.workflow_type as any) === 'auto_reply') {
                  if (plat === 'binance') return { icon: '💬', zh: '币安广场 · 自动互动', en: 'Binance Square Auto Engage', color: 'text-yellow-500 bg-yellow-500/10 border-yellow-500/30' };
                  return { icon: '💬', zh: '小红书 · 自动互动', en: 'XHS Auto Engage', color: 'text-cyan-500 bg-cyan-500/10 border-cyan-500/30' };
                }
