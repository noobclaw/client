/**
 * DouyinConfigWizard — 独立 wizard,字段隔离 (跟 TikTokConfigWizard 同结构,
 * 主色由粉改红,文案改为抖音上下文)。专为 douyin_auto_engage 场景设计:
 *
 *   - persona (人设)
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        // 抖音也没有 track / 关键词概念 — 传空但保留字段(后端 task store
        ...
        // Douyin-specific fields — orchestrator.js reads these.
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={500}
              placeholder={defaults.persona || (isZh ? '例: 喜欢刷抖音的普通观众,口语自然,不爹味' : 'e.g. casual Douyin viewer, natural tone')}
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              ...
              <ToggleCard
                ...
                desc={isZh ? 'AI 按视频文案生成' : 'AI from caption'}
                color="sky"
              />
            </div>
            {noActionEnabled && (
          ...
          {enableComment && (
            <div>
              ...
              <textarea
                ...
                rows={3}
                maxLength={400}
                placeholder={defaults.comment_prompt || (isZh ? '例: 用一句自然口语短评,不超过 30 字,中文素材回中文' : 'e.g. one casual short reaction; under 30 chars; match video language')}
                className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500/40 resize-y"
                disabled={saving}
              />
              ...
              <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">
                {isZh
                  ? '💡 默认中文回复;若视频文案是英文/日文等,AI 会自动匹配语言。不要带话题 # 或网址,容易被风控。'
                  : '💡 Defaults to Chinese; English / Japanese captions auto-switch. Skip hashtags & URLs to avoid spam flags.'}
              </div>
            </div>
          )}
          ...
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
            ⚠️ {isZh
              ? '抖音风控严格(尤其大号 / 蓝V),建议: ① 评论提示词写得自然,不要硬塞品牌词;② 关注默认关,需要时再开;③ 每天处理 3-8 个视频比较稳妥,过多易被限流。'
              : 'Douyin enforces strict anti-automation (esp. big or verified accounts). Tips: keep prompts natural; leave follow off by default; 3-8 videos per day is safe.'}
          </div>
