...
const KuaishouEngageCard: React.FC<CardProps> = ({ loading, scenario: _scenario, onConfigure, isZh }) => {
  return (
    <div className="relative rounded-2xl border border-orange-500/30 bg-gradient-to-br from-orange-500/10 via-amber-500/5 to-transparent p-5 overflow-hidden flex flex-col">
      ...
      <div className="relative flex flex-col flex-1">
        ...
        <button
          ...
          onClick={onConfigure}
          disabled={loading}
          className="w-full px-4 py-2.5 text-sm font-bold rounded-xl bg-orange-500 hover:bg-orange-600 disabled:opacity-40 disabled:cursor-not-allowed text-white shadow-lg shadow-orange-500/25 transition-all active:scale-95"
        >
          ⚡ {isZh ? '开始互动' : 'Start'} →
        </button>
      </div>
...
const KuaishouReplyFansCard: React.FC<CardProps> = ({ loading, scenario: _scenario, onConfigure, isZh }) => {
  return (
...
const KuaishouVideoDownloadCard: React.FC<CardProps> = ({ loading, scenario: _scenario, onConfigure, isZh }) => {
  return (
